...
class AccountPaymentModeLayout(models.Model):
    ...
    separator = fields.Char(
        help="Character that will be used to separate the values, default is no separator"
    )
    ...
    def _onchange_dynamic_placeholder(self):
        ...
        This method is called whenever the model_field_id, submodel_id or submodel_field_id fields are changed.
        ...
        if self.model_field_id:
            if self.model_field_id.ttype in ['many2one', 'one2many', 'many2many']:
                ...
                if model:
                    ...
                    self.expression = self._build_expression(self.model_id, self.model_field_id.name, sub_field_name)
            else:
                self.submodel_id = False
                self.submodel_field_id = False
                self.expression = self._build_expression(self.model_id, self.model_field_id.name, False)
        else:
            self.submodel_id = False
    ...
    def _build_expression(self, model, field_name, sub_field_name):
        """
        Build an expression string based on the given model, field name and subfield name.
    ...
    def generate_payment_file(self, order):
        ...
        :return: A tuple containing the payment file as bytes and the file name as a string.
        :rtype: tuple
        """
        self.ensure_one()
        payment_line = ""
...
class AccountPaymentModeLayoutLine(models.Model):
    ...
    infill = fields.Char(
        help="Character that will be used to fill the value to the length, default is space",
    )
